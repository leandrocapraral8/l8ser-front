...
async function deleteCustomerAndCloseModal() {
  isModalOpen.value = false
  try {
    const response = await $axios.delete(`/customers/${selectedCustomerId.value}`)
      ...
      <div class="p-4 md:p-6">
        <div class="mx-auto w-full max-w-xs text-center">
          ...
          <p class="font-alt text-muted-500 dark:text-muted-400 text-sm leading-5">
            Are you sure you want to delete the order with ID: <strong>{{ selectedCustomerId }}</strong>?
          </p>
        </div>
      </div>
        ...
        <div class="p-4 md:p-6">
          <div class="flex gap-x-2">
            ...
            <BaseButton
              color="primary"
              variant="solid"
              @click="deleteCustomerAndCloseModal"
            >
              Accept
            </BaseButton>
          </div>
