...
definePageMeta({
  ...
  preview: {
    title: 'Edit customer',
    description: 'For editing a customer information',
...
onBeforeMount(async () => {
  try {
    const response = await $axios.get(`http://localhost:8006/customers/${customerId}`)
    ...
    resetForm({
      values: {
        ...
        customer: {
          ...
          freshdesk_id: data.value?.freshdesk_id || '',
        },
      },
    })
  }
...
const onSubmit = handleSubmit(
  async (values) => {
    ...
    const payload = {
      ...
      active: values.customer.active,
      freshdesk_id: values.customer.freshdesk_id,
    }
    ...
    try {
      const response = await $axios.patch(`/customers/${customerId}/`, payload, {
        headers: {
      ...
      toaster.show({
        title: 'Success',
        message: `Your customer information has been updated!`,
        color: 'success',
        icon: 'ph:check',
        closable: true,
      })
...
watch(() => values.customer.active, (newValue) => {
  console.log('Active value changed:', newValue)
})
</script>
      ...
      <div class="flex items-center justify-between p-4">
        <div>
          ...
          <BaseText size="xs" class="text-muted-400">
            Edit general info about this customer
          </BaseText>
        </div>
        <div class="flex items-center gap-2">
          <BaseButton class="w-24" to="/customers/">
            Cancel
          </BaseButton>
          <BaseButton
            color="primary"
    ...
    <div class="p-4 md:p-6">
      <div class="mx-auto w-full max-w-xs text-center">
        ...
        <p class="font-alt text-muted-500 dark:text-muted-400 text-sm leading-5">
          Are you sure you want to update the information for customer with ID: <strong>{{ customerId }}</strong>?
        </p>
      </div>
    </div>
